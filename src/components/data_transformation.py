...
class DataTransformation:
    ...
    def get_data_transformation_object(self):
        try:
            numeric_columns = ['LotArea', 'OverallQual', 'OverallCond', 'YearBuilt',
                               'YearRemodAdd', 'TotalBsmtSF', 'GrLivArea', 'BsmtFullBath',
                               'TotRmsAbvGrd', 'Fireplaces', 'GarageArea', 'OpenPorchSF', ]
            categorical_columns = ['Street', 'LotShape', 'LandSlope', 'Neighborhood', 'Foundation', 'CentralAir', ]
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            target_columns_name = "SalePrice"

            input_feature_train_df = train_df.drop(columns=[target_columns_name], axis=1)
            target_feature_df = train_df[target_columns_name]
            ...
            train_arr = np.c_[input_features_train_arr, np.array(target_feature_df)]
